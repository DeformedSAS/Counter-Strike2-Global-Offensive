...
var HudWinPanel = (function () {
    ...
    function _Init() {
        if (_m_bInit) return;
        $.RegisterForUnhandledEvent('HudWinPanel_ShowRoundImpactScoreReport', _ShowRoundEndReport);
    ...
    function _OnReceivePlayerHurt(attackerXuid, victimXuid, damage) {
        if (!_m_ListeningForGameEvents) return;
        if (!_m_bCanvasIsReady) {
            $.Schedule(0.5, () => _OnReceivePlayerHurt(attackerXuid, victimXuid, damage));
            return;
        }
        if (_m_localXuid != attackerXuid && _m_localXuid != victimXuid) return;
    ...
    function _OnReceiveMVP(xuid) {
        if (!_m_ListeningForGameEvents) return;
        if (!_m_bCanvasIsReady) {
            $.Schedule(0.5, () => _OnReceiveMVP(xuid));
            return;
        }
    ...
    function _ConvertToLocalOdds(terroristOdds) {
        if (_m_teamPerspective == 2) return terroristOdds;
        else return (100 - terroristOdds);
    }
    ...
    function _ShowRoundEndReport(msg) {
        ...
        if (!_m_elCanvas.IsSizeValid()) {
            $.Schedule(0.5, () => _ShowRoundEndReport(msg));
            return;
        }
        ...
        const arrEvents = Object.values(msg['all_ris_event_data']);
        ...
        const plotPoints = [startPlotPoint];
    ...
    function _ExtractLivingEnemies(arrEvents) {
        ...
        arrEvents.forEach(function (oEvent) {
            ...
            if (isLivingPlayer && isEnemy) arrResults.push(oEvent);
        });
        return arrResults;
    }
    ...
    function _ProcessTimelineEvents(arrEvents, points, plotPoints, nStartingOdds) {
        ...
        arrEvents.forEach(function (oEvent, index) {
            ...
            if (index == 0) delta = oEvent['terrorist_odds'] - nStartingOdds;
            else delta = oEvent['terrorist_odds'] - arrEvents[index - 1]['terrorist_odds'];
            ...
            Scheduler.Schedule(delay, () => {
                ...
                if (loopingSfxHandle) $.StopSoundEvent(loopingSfxHandle, 0.1);
    ...
    function _FindDamageDataForPlayer(oEvent, xuid) {
        ...
        const returnObj = {};
        const arrDamages = Object.values(oDamageData);

        for (let i = 0; i < arrDamages.length; i++) {
            if (arrDamages[i]['target_xuid'] == xuid) {
                Object.assign(returnObj, arrDamages[i]);
            }
        }
        return returnObj;
    }
    ...
    function _UpdateDamage(xuid, healthRemoved, numHits, returnHealthRemoved, returnHits) {
        ...
        if ((elDamage.returnHealthRemoved > 0) || (elDamage.healthRemoved > 0)) {
            ...
            if (BEAM_ONLY_ON_DAMAGE) {
                ...
                if (elTeamColorBar) elTeamColorBar.RemoveClass('prereveal');
            }

            const dmgDelay = 0.1;
            Scheduler.Schedule(dmgDelay, () => {
                if (elDamage && elDamage.IsValid()) elDamage.RemoveClass('prereveal');
            });
        }
    }
    ...
    function _AddDamageToDamagePanel(oEvent, plotPoint) {
        ...
        if (oDamage && Object.keys(oDamage).length > 0) {
            ...
            const nReturnHits = oDamage['num_return_hits'] || 0;
    ...
    function _DecoratePoint(oEvent, plotPoint) {
        ...
        if (elDeath) elDeath.visible = false;
        ...
        if (victimData) {
            ...
            if (elEventChance) elEventChance.visible = isDead;
            if (elDeath) elDeath.visible = isDead;
            ...
            if (elEventMain) {
                const jsAvatarTeamColor = elEventMain.FindChildTraverse('JsAvatarTeamColor');
                if (jsAvatarTeamColor) {
                    jsAvatarTeamColor.style.washColor = 'rgb(' + rgbColor + ')';
                    jsAvatarTeamColor.visible = !isBot;
                }
            }
        } else if (objectiveData) {
            const elAvatarImage = elEventPlot.FindChildTraverse('Avatar');
            if (elAvatarImage) elAvatarImage.visible = false;

            let src = "";
            let bEventCT = false;
            switch (objectiveData['type']) {
                case 0: src = "file://{images}/icons/ui/bomb_c4.svg"; bEventCT = false; break;
                case 1: src = "file://{images}/icons/ui/bomb.svg"; bEventCT = false; break;
                case 2: src = "file://{images}/icons/equipment/defuser.svg"; bEventCT = true; break;
                case 3: src = "file://{images}/icons/ui/time_exp.svg"; bEventCT = true; break;
            }
        ...
        if (chance == 100) {
            ...
            if (elEventChance) elEventChance.FindChildTraverse('EventChanceNumber').style.color = '#ffffff';
        } else if (chance == 0) {
            elEventPlot.SetDialogVariable('chance', $.Localize('ris_loss'));
            if (elEventChance) elEventChance.FindChildTraverse('EventChanceNumber').style.color = '#ffffff';
        } else {
            elEventPlot.SetDialogVariable('chance', deltaSymbol + chance + '%');
            if (elEventChance) elEventChance.FindChildTraverse('EventChanceNumber').style.color = _RemapToTeamColorRGB(chance - _m_prevChance, -20, 20);
        }

        elEventPlot.style.x = plotPoint[0] + "px";
        elEventPlot.style.y = plotPoint[1] + "px";

        if (elEventMain && elEventMain.IsValid()) elEventMain.RemoveClass('prereveal');
        if (elEventChance && elEventChance.IsValid()) elEventChance.RemoveClass('prereveal');
        if (elDeath && elDeath.IsValid()) elDeath.RemoveClass('prereveal');
    ...
    function _Reset() {
        ...
        var elWinnerPlaque = $.GetContextPanel().FindChildTraverse('WinnerPanel');
        if (elWinnerPlaque) elWinnerPlaque.AddClass('prereveal');

        var elMVPPanel = $.GetContextPanel().FindChildTraverse('RISMVPPanel');
        if (elMVPPanel) elMVPPanel.visible = false;

        var elAvatar = elMVPPanel ? elMVPPanel.FindChildTraverse('RISMVPAvatar') : null;
        if (elAvatar) elAvatar.AddClass('prereveal');
